...
from predict_score import topk_accs, compute_metric, dsd_func, dsd_func_mundo
import re
import pandas as pd
...
# python run_method.py --ppiA ../data/intact_output/fly.s.tsv --ppiB ../data/intact_output/bakers.s.tsv --nameA fly --nameB bakers --dsd_A_dist fly-dsd-dist.npy --dsd_B_dist yeast-dsd-dist.npy --thres_dsd_dist 10 --json_A fly.json --json_B yeast.json --mds_A fly-isomap.npy --mds_B yeast-isomap.npy --mds_r 100 --landmarks_a_b isorank_fly_bakers.tsv --no_landmarks 450 --model "yeast->fly.sav" --transformed_b_a "yeast->fly_emb.npy" --mds_dist_a_b "dist_fly->yeast.npy" --compute_go_eval --kA 10 --kB 10 --metrics top-1-acc --output_file testfly_yeast.tsv --go_A ../data/go/fly.output.mapping.gaf --go_B ../data/go/bakers.output.mapping.gaf  # only apply --compute_isorank if you have not generated isorank
...
def main(args):
    ...
    if args.compute_go_eval:
            ...
            for metric in args.metrics.split(","):
                ...
                for kA in kAs:
                    ...
                    for kB in kBs:
                        ...
                        scores, _ = compute_metric(dsd_func_mundo(DSDA, DISTM, gomapsB[go], k_other=kB),
        ...
        resultsdf.to_csv(args.output_file, sep = "\t", index = None)
    return
...
if __name__ == "__main__":
    ...
    # Compute distance ? 
    if args.mds_dist_a_b is None or (not os.path.exists(args.mds_dist_a_b)):
        todos = ["mds_dist_a_b"] + todos
        
        if args.transformed_b_a is None or (not os.path.exists(args.transformed_b_a)):
            todos = ["transformed_b_a"] + todos
        
            if args.model is not None or not os.path.exists(args.model):
                todos = ["model"] + todos
                
                if args.compute_isorank:
                    todos = ["isorank"] + todos
                    
                if args.mds_A is None or (not os.path.exists(args.mds_A)):
                    todos = ["mds_A"] + todos
                    
                    if args.dsd_A_dist is None or (not os.path.exists(args.dsd_A_dist)):
                        todos = ["dsd_A_dist"] + todos
                    
                if args.mds_B is None or (not os.path.exists(args.mds_B)):
                    todos = ["mds_B"] + todos
                    
                    if args.dsd_B_dist is None or (not os.path.exists(args.dsd_B_dist)):
                        todos = ["dsd_B_dist"] + todos
