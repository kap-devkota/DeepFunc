...
def getargs():
    ...
    parser.add_argument("--thres_dsd_dist", type=float, default=100,
    ...
    parser.add_argument("--json_A", default=None,
                        help="Protein annotation to Matrix index in json format for DSD and SVD matrix of A. If not present, the JSON file will be computed at this location")
    parser.add_argument("--json_B", default=None,
                        help="Protein annotation to Matrix index in json format for DSD and SVD matrix of B. If not present, the JSON file will be computed at this location")

    # SVD
    parser.add_argument("--svd_A", default=None,
                        help="SVD representation of the species A. If not present, the SVD representation will be produced at this location")
    parser.add_argument("--svd_B", default=None,
                        help="SVD representation of the species B. If not present, the SVD representation will be produced at this location")
    ...
    parser.add_argument("--model", default=None,
                        help="Location of the trained model. If not provided, the trained model is saved at this location")
    parser.add_argument("--svd_dist_a_b", default=None,
                        help="Location of the SVD COSINE distance between A and transformed B. If not present, the distance will be created at this location")
...
def predict_for_unknown(prediction_func, allprots, true_go_map):
    predictprots = list(filter(lambda prot: prot not in true_go_map or not true_go_map[prot], allprots))
    trainprots = list(filter(lambda prot: prot not in predictprots, allprots))
...
def main(args):
    ...
    if args.compute_go_eval:
        ...
        for go in gos:
            ...
            inv_nmapA = {v: k for k, v in nmapA.items()}
            for kA in kAs:
                pred_map_dsd = predict_for_unknown(dsd_func(DSDA, k=kA), list(range(len(nmapA))), gomapsA[go][0])
                for kB in kBs:
                    print(f"!!!!!!!!!!!!!!!!!!! GO: {go}, DSD, kA: {kA}, kB: {kB}")
                    pred_map_mundo = predict_for_unknown(
                        dsd_func_mundo(DSDA, DISTS, gomapsB[go][0], k=kA, k_other=kB, weight_other=args.wB),
                        list(range(len(nmapA))),
                        gomapsA[go][0])
                    pred_map_diff = {}
                    for prot_idx in pred_map_mundo:
                        if pred_map_mundo[prot_idx]:
                            for mundo_pred in list(pred_map_mundo[prot_idx]):
                                if mundo_pred not in pred_map_dsd[prot_idx]:
                                    pred_map_diff[inv_nmapA[prot_idx]] = mundo_pred
                                    break
                    print(pred_map_diff)
    return
